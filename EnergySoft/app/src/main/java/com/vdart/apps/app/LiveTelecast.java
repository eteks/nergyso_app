...
public class LiveTelecast extends AppCompatActivity implements Download_data.download_complete{
    Toolbar toolbar;
    View view;
    private static final String TAG = "EventMain";
    String listValue = "NULL";
    LiveTelecast.PaginationAdapter adapter;
...
//        list.setAdapter(NewsAdapter);
        ...
        adapter = new LiveTelecast.PaginationAdapter(this);
...
//        final List<News> newsList = News.createMovies(adapter.getItemCount());
        Download_data download_data = new Download_data((Download_data.download_complete) this);
        download_data.download_data_from_link(LIVE_URL);
...
//                else isLastPage = true;
    ...
    public void get_data(String data)
    {
        try {
            ...
            for (int i = 0 ; i < data_array.length() ; i++)
            {
                ...
                add.setImage("https://img.youtube.com/vi/" + splitted_gallery[3] + "/0.jpg");
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        adapter.add(add);
    ...
    public class PaginationAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            switch (viewType) {
                ...
                case LOADING:
                    ...
                    viewHolder = new LiveTelecast.PaginationAdapter.LoadingVH(v2);
                    break;
            }
            return viewHolder;
        }
        ...
        private RecyclerView.ViewHolder getViewHolder(ViewGroup parent, final LayoutInflater inflater) {
            ...
            viewHolder = new LiveTelecast.PaginationAdapter.EventVH(v1);
            final View.OnClickListener mOnClickListener = new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Event event = eventList.get(viewHolder.getAdapterPosition());
                    Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("vnd.youtube://" + event.getType()));
                    startActivity(intent);
                }
            };
...
//        Movie movie = movies.get(position);
            ...
            switch (getItemViewType(position)) {
                case ITEM:
                    LiveTelecast.PaginationAdapter.EventVH eventVH = (LiveTelecast.PaginationAdapter.EventVH) holder;
